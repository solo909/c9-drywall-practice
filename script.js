...
function startQuiz() {
  ...
  let pool = window.QUESTION_BANK.filter(q => setValue === 'mixed' || q.set === setValue);
...
function showNeutral(lang) {
  statusText.className = 'status-muted';
  statusText.textContent = lang === 'es'
    ? 'Escoge una respuesta para ver feedback.'
...
function buildExplanation(q, selected, lang) {
  ...
  return base + `<p>${topicTip(q, lang)}</p>`;
}
